...
const entrepriseFiltre = ref(props.idEntreprise ? Number(props.idEntreprise) : '')
...
const kpiQuiz = ref({ tauxCompletion: 0, tauxEliminationP1: 0, tauxClicOnedoc: 0 })
...
const questionPrincipaleNonEligibilite = computed(() => {
  ...
  return questionsQuiz.value.reduce((max, q) => q.mauvaises > max.mauvaises ? q : max, questionsQuiz.value[0])
})

async function chargerStats() {
  try {
    const url = entrepriseFiltre.value
      ? `/analytics-stats?company_id=${entrepriseFiltre.value}`
      : '/analytics-stats'
...
<template>
  <DashboardLayout vueCourante="analytics" :allerVers="allerVers">
    ...
    <div class="mb-6 flex flex-wrap items-center gap-3">
      <h1 class="font-sans text-h2 font-bold text-texte-secondary">{{ titrePage }}</h1>
      <button
    ...
    <div class="mb-6 flex flex-wrap items-end gap-4">
      ...
      <div class="flex flex-col gap-1">
        ...
        <select v-model="entrepriseFiltre"
          ...
          <option v-for="e in entreprisesDisponibles" :key="e.id" :value="e.id">{{ e.nom }}</option>
        </select>
      </div>
    </div>
