...
const logoUrl = computed(() => props.company.collections?.[0]?.logo_url ?? null)
const labelStartDate = computed(() => props.company.labels?.[0]?.pivot?.start_date ?? null)
const badgeImg = '/images/' + 'label.png'
...
<template>
  <div class="flex flex-col items-center gap-4 rounded-[10px] bg-violet-50 p-2.5 shadow-[0_4px_4px_1px_rgba(104,23,100,0.30)] lg:gap-[22px]">
    <!-- Logo + Badge Label -->
    <div class="flex w-full items-center justify-between px-1">
      <!-- Logo entreprise -->
      <div class="flex h-[42px] w-[75px] items-center justify-center overflow-hidden lg:h-[50px]">
        <img
          v-if="logoUrl"
          :src="logoUrl"
          :alt="company.name"
        ...
        />
        <div v-else class="flex h-10 w-16 items-center justify-center rounded bg-violet-100 text-xs text-violet-600">
          {{ company.name }}
        </div>
      </div>
    ...
    <div class="flex flex-col items-start gap-2 rounded-[10px]">
      ...
      <div class="hidden items-center gap-5 lg:flex">
        ...
        <span class="py-2.5 text-center font-sans text-regular text-violet-950">
          {{ company.size_label }}
        </span>
      </div>
